...
async function listGitHubVesti() {
  const headers = {
    Authorization: `Bearer ${GITHUB_TOKEN}`,
    "X-GitHub-Api-Version": "2022-11-28",
  };
  ...
  const rootRes = await fetch(
    `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/content/vesti?ref=${GITHUB_BRANCH}`,
    { headers }
  );
  ...
  for (const entry of entries.filter((e) => e.type === "dir")) {
    const folderRes = await fetch(
      `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${enc(`content/vesti/${entry.name}`)}?ref=${GITHUB_BRANCH}`,
      { headers }
    );
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (searchParams.get("images") === "1" && slug) {
      ...
      if (IS_LOCAL) {
        const publicDir = path.join(ROOT, "public", "content", "vesti", slug);
      ...
      const res = await fetch(
        `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${enc(`content/vesti/${folder}`)}?ref=${GITHUB_BRANCH}`,
        { headers: ghHeaders }
      );
      ...
      const images = files
        ...
        .map((f) => `/content/vesti/${slug}/${f.name}`);
      return NextResponse.json({ images });
    }

    // --- Jedna vest (za edit) ---
    if (folder && slug) {
      const mdxRelPath = `content/vesti/${folder}/${slug}.mdx`;
    ...
    // --- Lista svih vesti ---
    if (IS_LOCAL) {
      const vestiDir = path.join(ROOT, "content/vesti");
      if (!fs.existsSync(vestiDir)) return NextResponse.json({ items: [] });

      const items: { folder: string; slug: string; title: string; date: string; author: string; excerpt: string; arhivirano: boolean }[] = [];

      for (const entry of fs.readdirSync(vestiDir, { withFileTypes: true })) {
        if (!entry.isDirectory()) continue;
        const folderPath = path.join(vestiDir, entry.name);
    ...
    const items = await listGitHubVesti();
    return NextResponse.json({ items });
  } catch (err) {
    console.error(err);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    await putFile(mdxRelPath, Buffer.from(mdxContent).toString("base64"), `Admin: izmeni vest "${title}"`, fileSha);
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error(err);
    return NextResponse.json({ error: err instanceof Error ? err.message : String(err) }, { status: 500 });
  ...
  for (const file of files.filter((f) => f.type === "file")) {
    await fetch(
      `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${enc(file.path)}`,
      {
        ...
        body: JSON.stringify({ message: `Admin: obriši vest "${folderLabel}"`, sha: file.sha, branch: GITHUB_BRANCH }),
      }
    );
  }
}
    ...
    if (Array.isArray(images) && images.length > 0) {
      ...
      if (IS_LOCAL) {
        for (const filename of filenames) {
          const pub = path.join(ROOT, "public", "content", "vesti", slug, filename);
          const cnt = path.join(ROOT, "content", "vesti", folder, filename);
      ...
      for (const filename of filenames) {
        const msg = `Admin: obriši sliku "${filename}" iz vesti "${slug}"`;
        await deleteGitHubFile(`public/content/vesti/${slug}/${filename}`, msg);
        await deleteGitHubFile(`content/vesti/${folder}/${filename}`, msg);
      }
      return NextResponse.json({ success: true });
    }

    // --- Brisanje cele vesti ---
    const contentFolder = `content/vesti/${folder}`;
    const publicFolder  = `public/content/vesti/${slug}`;
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (IS_LOCAL) {
      const contentFolder = path.join(ROOT, `content/vesti/${folder}`);
    ...
    if (IS_LOCAL) {
      for (let i = 0; i < galleryFiles.length; i++) {
        ...
        localWrite(`content/vesti/${folder}/${filename}`, buffer);
        localWrite(`public/content/vesti/${slug}/${filename}`, buffer);
      }
      return NextResponse.json({ success: true });
    }
    ...
    for (let i = 0; i < galleryFiles.length; i++) {
      ...
      const contentImgPath = `content/vesti/${folder}/${filename}`;
      const publicImgPath  = `public/content/vesti/${slug}/${filename}`;

      const cSha = await getFileSha(contentImgPath);
      await putFile(contentImgPath, base64, `Admin: dodaj galeriju za vest "${slug}"`, cSha);

      const pSha = await getFileSha(publicImgPath);
      await putFile(publicImgPath, base64, `Admin: dodaj galeriju za vest "${slug}"`, pSha);
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (IS_LOCAL) {
      if (fs.existsSync(path.join(ROOT, `content/vesti/${folderName}`))) {
        return NextResponse.json({ error: `Vest sa naslovom "${title}" već postoji.` }, { status: 409 });
      }
    } else {
      const existingSha = await getFileSha(`content/vesti/${folderName}/${slug}.mdx`);
      if (existingSha) {
        return NextResponse.json({ error: `Vest sa naslovom "${title}" već postoji.` }, { status: 409 });
      }
    }
    ...
    if (IS_LOCAL) {
      const contentFolder = `content/vesti/${folderName}`;
      const publicFolder = `public/content/vesti/${slug}`;
    ...
    const mdxPath = `content/vesti/${folderName}/${slug}.mdx`;
    const mdxSha = await getFileSha(mdxPath);
    await putFile(mdxPath, Buffer.from(mdxContent).toString("base64"), `Admin: dodaj vest "${title}"`, mdxSha);
    ...
    for (const { file, index } of allImages) {
      ...
      const contentImgPath = `content/vesti/${folderName}/${filename}`;
      const publicImgPath = `public/content/vesti/${slug}/${filename}`;

      const contentSha = await getFileSha(contentImgPath);
      await putFile(contentImgPath, base64, `Admin: dodaj sliku za vest "${title}"`, contentSha);

      const publicSha = await getFileSha(publicImgPath);
      await putFile(publicImgPath, base64, `Admin: dodaj sliku za vest "${title}"`, publicSha);
    }
