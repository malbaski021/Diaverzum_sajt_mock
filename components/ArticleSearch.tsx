...
export default function ArticleSearch({ articles, section, placeholder = "Pretraži..." }: Props) {
    ...
    <>
      ...
      {query && (
        <p className="text-gray-500 mt-4 text-sm">
          {filtered.length > 0
            ? `${filtered.length} rezultat(a) za "${query}"`
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {filtered.map((article) => (
            <ArticleCard key={article.slug} article={article} section={section} />
          ))}
        </div>
      ) : (
        <div className="text-center py-24 text-gray-400 mt-6">
          <p className="text-5xl mb-4" aria-hidden="true">{section === "vesti" ? "📭" : "✍️"}</p>
          <p className="text-lg font-medium">
            {query ? "Nema rezultata" : section === "vesti" ? "Vesti uskoro..." : "Blog tekstovi uskoro..."}
          </p>
        </div>
      )}
    </>
